    ...
    <div className='grid grid-cols-1 md:grid-cols-4 md:max-w-[1200px] relative xl:max-w-[1440px] m-auto px-5 pt-14 pb-20 md:pb-14'>
      ...
      <div className='order-1 md:order-3 mb-12 md:mb-0 md:h-[150px] flex justify-center items-start md:flex-col md:justify-between md:items-end'>
        <form className='flex gap-2 flex-wrap w-[375px]' onSubmit={handleSubmit(sendForm)}>
          ...
          />
          <input type="submit" value="Go" className='text-[14px] w-[25%] uppercase md:normal-case py-2 px-7 rounded-full bg-primary text-lightGray font-bold cursor-pointer hover:opacity-80'/>
          {errors.email && <span className='text-red-700 italic text-[12px] ml-4'>{errors.email.message}</span>}
        </form>
        <div className='absolute md:relative bottom-5 left-[50%] md:left-0 -translate-x-[50%] md:translate-x-0'>
          <span className='text-[12px] text-lightGray/40 whitespace-nowrap'>Copyright 2020. All Rights Reserved</span>
        </div>
      </div>
    </div>
  )
